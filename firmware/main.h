...
#define TX_BUF_FRAME_END	(TX_BUF_POSTFIX_START + TX_BUF_POSTFIX_LENGTH)
/* checksum parameters */
#define TX_BUF_CHECKSUM_BEGIN	sizeof(SYNC_PREFIX "$$") - 1
#define TX_BUF_CHECKSUM_END	TX_BUF_CHECKSUM_START - 1

#define TX_BUF_MAX_LENGTH	(TX_BUF_START_OFFSET + \
				SENT_ID_LENGTH_MAX + 1 + TIME_LENGTH + 1 + \
				LAT_LENGTH + 1 + 1 + LON_LENGTH + 1 + 1 + \
				ALT_LENGTH_MAX + 1 + SAT_LENGTH + 1 + VOLT_LENGTH + 1 + \
				OP_STAT_LENGTH + 1 + TEMP_LENGTH + 1 + 1 + \
				CHECKSUM_LENGTH + TX_BUF_POSTFIX_LENGTH)
